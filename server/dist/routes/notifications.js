...
router.patch('/:id/read', auth_1.auth, async (req, res) => {
    try {
        const notification = await Notification_1.default.findOneAndUpdate({ _id: req.params.id, recipient: req.user?.id }, { isRead: true }, { new: true });
        if (!notification)
...
// @access  Private
router.post('/seed', auth_1.auth, async (req, res) => {
    try {
        ...
        const sampleListing = await Listing_1.default.findOne();
        ...
        await Notification_1.default.insertMany(testNotifications);
